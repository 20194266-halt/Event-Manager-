		...
		for (int i = 1; i < line.length(); i++) {
			...
			if (line[i] == '|') {
				...
				if (cnt == 1) {
					...
					cout << "Name " << eventNameMap[eventId] << endl;
					break;
				}
...
void messageHandling(socketInfo &client, socketInfo* sock, DWORD &index, DWORD &nEvents, WSAEVENT* events)
{
	...
	for (int i = 0; i < recvMessage.size(); i++) {
		...
		{
			//string code = logoutHandling(tmpStr, mp, sock);
		}
		...
		{
			int code = requestHandling(tmpStr);
			sendMessage(client.clientSock, to_string(code));
			//passed
		}
		else if (header == "REQUESTREPLY")
		{
			int code = requestReplyHandling(tmpStr);
			sendMessage(client.clientSock, to_string(code));
			//passed
		}
