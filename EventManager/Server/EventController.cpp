...
//map<string, string> nameAndPass;//key: username
...
void listUserAndEvent() {
	...
	if (new_file.is_open()) {
		while (getline(new_file, tmpStr)) {
			...
			pos = tmpStr.find(key);
			string userName = tmpStr.substr(0, pos);
			string eventName = tmpStr.substr(pos + 1, tmpStr.length() - pos);
...
int requestHandling(string s) {
	int cnt = 0;
	string sender; //REQUEST nguoigui|nguoinhan|ttsk
	string receiver;
	string eventId;
	string tmpStr;
...
int requestReplyHandling(string s) {
	...
	if (checkEventId(eventName) == 0) {
		cout << "event doesn't exist" << endl;
		return 52; //event doesnt exist
	}
	if (checkUserName(receiver) == 0) { //EVENT_REQUEST_REPLY nguoigui|nguoinhan|tensk|reply(OK/DENY)
		cout << "receiver doesn't exist" << endl;
		return 53; // receiver doesnt exist
	}
	if (reply == "OK") {
		cout << "accecpt request" << endl;
		ofstream ofs("RequestReply.txt");
		string pushStr = receiver + "|" + sender + "|" + eventName + "|" + reply + "|";
		ofs.write(pushStr.data(), pushStr.size());
		return 50;//reply successfully
	}
	if (reply == "DENY") {
		cout << "deny request" << endl;
		ofstream ofs("RequestReply.txt");
		string pushStr = receiver + "|" + sender + "|" + eventName + "|" + reply + "|";
		ofs.write(pushStr.data(), pushStr.size());
		return 50;//reply successfully
	...
	for (int i = 0; i < s.length(); i++) {
		if (s[i] == '|') {
			...
			if (cnt == 1) {
				...
				//wrong creator name or eventId
				if (eventNameMap[eventId] != eventCreator) {
					cout << "3" << endl;
					return "99";
				}
			}
...
string inviteReplyHandling(string s, map<string, string>&nameAndPass, map<string, string>&eventNameMap) {
	//INVITEREPLY Y|E|eventId|OK/DENY|
	ofstream outfile;
